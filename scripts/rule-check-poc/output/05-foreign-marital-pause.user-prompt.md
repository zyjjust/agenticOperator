...
```json
{
  "upload_id": "upl_005_c05-zhou",
  "candidate_id": "c05-zhouqi-foreign-data",
  "resume_id": "res_005",
  "employee_id": "EMP_REC_007",
  "bucket": "recruit-resume-raw",
  "object_key": "2026/05/upl_005_c05-zhou.pdf",
  "filename": "c05-zhouqi-foreign-data.pdf",
...
### 3.3 部门级规则 (本次 business_group="CDG", studio="无" — 2 条)

#### 规则 10-42:CDG事业群6个月回流冷冻期绝对拦截 [终止级]

**触发条件**:目标岗位归属腾讯CDG事业群，且候选人简历中存在腾讯或腾讯外包的历史工作经历。

**判定逻辑**:在匹配腾讯CDG事业群的岗位时，系统检查候选人简历的详细工作履历及职责描述。查找所有涉及腾讯或腾讯外包的工作经历，并提取其最近一次从腾讯离职的具体日期。计算该离职日期距离当前日期的间隔时间。如果该离职日期距今不足6个月（即使候选人在离开腾讯后又有其他非腾讯公司的短暂工作经历），系统必须立即阻断该候选人的岗位推荐流程，且不提供任何人工审核放行机制；如果候选人没有任何腾讯相关工作经历，或者最近一次从腾讯离职的时间已满6个月及以上，则正常继续匹配流程。

**命中时的输出动作**:
- 在 `rule_flags` 中加一条 `{rule_id: "10-42", severity: "terminal", applicable: true, result: "FAIL", evidence: "<引用简历原文>"}`
- `drop_reasons` 加 `"10-42:<short_code>"`,`next_action`="block"

#### 规则 10-53:非IEG事业群跳过内部技术面试 [仅记录]
...
**命中时的输出动作**:
- 在 `rule_flags` 中加一条 `{rule_id: "10-53", severity: "flag_only", applicable: true, result: "PASS", evidence: "<引用简历原文>"}`
- 仅在 `resume_augmentation` 文本里追加一行 flag,不写 drop/pause reasons,`next_action`="continue"
