...
function emitSnapshot(obj: ActionObjectV4, actionName: string, typesImport: string): string {
  ...
  const placeholderNote = isMatchResumeAction({ name: actionName })
    ? "// prompt contains three hierarchical placeholders: {{CLIENT}} / {{JOB}} / {{RESUME}}.\n// Call fillRuntimeInput(obj, { kind: \"matchResume\", client, job, resume }) at consumption time."
    : "// prompt contains a single {{RUNTIME_INPUT}} placeholder.\n// Call fillRuntimeInput(obj, <string-or-object>) at consumption time.";
