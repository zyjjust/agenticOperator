...
//
// Spec source: "RAAS Dashboard 内部 API 接入说明" (2026-04-28)
//
// Endpoint:    GET {RAAS_INTERNAL_API_URL}/api/v1/internal/requirements
// Auth:        Authorization: Bearer ${RAAS_AGENT_API_KEY}
// Used by:     matchResume agent to fetch authoritative structured
//              requirement data (must_have_skills[], nice_to_have_skills[],
//              degree_requirement, work_years, etc.) before scoring a
//              resume against it.
//
// Why this exists alongside REQUIREMENT_LOGGED events:
//   - REQUIREMENT_LOGGED.payload.raw_input_data has unstructured strings
//     (job_responsibility, job_requirement) that need LLM expansion.
//   - This API returns already-structured arrays (must_have_skills[],
//     nice_to_have_skills[]) that the matcher can use directly without
//     re-parsing.
//   - The API is also the canonical source of TRUTH after RAAS-side
//     edits (HSM may amend the requirement after the initial event).
//
// Errors per spec §5:
//   401 — AGENT_API_KEY mismatch / missing header
//   400 — missing claimer_employee_id
//   503 — RAAS not ready, retry later
//   * other 4xx/5xx — raise RaasInternalApiError

const BASE = process.env.RAAS_INTERNAL_API_URL ?? "";
const KEY = process.env.RAAS_AGENT_API_KEY ?? "";
const TIMEOUT_MS = Number(process.env.RAAS_INTERNAL_API_TIMEOUT_MS ?? 30_000);
...
/**
 * Fetch a specific requirement by its `job_requisition_id`.
 *
 * The RAAS API doesn't have a by-id GET endpoint per spec — only the
 * paginated list. We page through the claimer's requirements and
 * return the matching one. For a recruiter with hundreds of claims
 * this could be slow; in practice cache the (employee_id, jr_id) →
 * RaasRequirement mapping if it shows up in profiling.
 */
export async function findRequirementById({
  ...
  while (true) {
    ...
    const hit = r.items.find((it) => it.job_requisition_id === jobRequisitionId);
    if (hit) return hit;
    if (r.items.length === 0 || page >= r.total_pages) return null;
    page += 1;
  }
}

/**
 * Flatten a RaasRequirement into a single block of plain text suitable
 * for RoboHire `/match-resume` — same shape as the LLM-generated JD
 * text but with the AUTHORITATIVE structured arrays merged in.
 */
